...
def get_pricing_formula():
    return {
        "formula": "单次审计费用 = 基础审计费 (按档位 × 复杂度系数 × 资产池系数) + (可选赏金池 × 10%-20% 抽成) + (触发时的争议费：仲裁押金 + 委员会启动费)",
        "discount_formula": "最终应付 = (基础费用 + 赏金抽成 + 争议费) × (1 - 代币折扣%) × (1 - 质押折扣除%)",
        "components": {
            "base_audit_fee": "按档位、复杂度、资产池规模三要素自动计算",
            "bounty_boost": "项目方额外注资赏金池，平台抽取 10%-20% 佣金",
            "dispute_fee": "包含仲裁押金 ($2,000-$5,000) 和委员会启动费 ($500-$1,500)，仅在争议时触发",
        },
        "discounts": {
...
PAYMENT_PROOFS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "payment_proofs")
