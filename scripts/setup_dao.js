...
async function main() {
  ...
  const daoAddr = process.env.NEW_DAO_ADDRESS || "0x16CD469E70e1f25F6403A9380034b23391EDec35";
  ...
  // Read team manager from DAO
  const teamMgrAddr = await dao.teamManager();
  console.log("TeamManager:", teamMgrAddr);

  await doTx("TeamManager.setDaoContract", async () => {
    const teamMgr = await ethers.getContractAt("AuditTeamManager", teamMgrAddr);
    return teamMgr.setDaoContract(daoAddr);
  });

  await doTx("setAuditTeam", () => dao.setAuditTeam(multisigAddr));
  ...
  console.log("\n===== All setup complete for:", daoAddr, "=====");
}
